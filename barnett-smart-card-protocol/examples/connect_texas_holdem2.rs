...
async fn main() -> anyhow::Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Conectado como peer: {}", connection.local_peer_id());
    ...
    // Enviar la clave pública del jugador
    // let pk_bytes = to_bytes![andrija.pk]?;
    let pk_bytes = P2PConnection::serialize_canonical(&andrija.pk)?;
    connection.send_public_key(&pk_bytes).await?;

    // Recibir claves públicas de otros jugadores
    let mut player_keys = Vec::new();
    let mut all_peer_ids = vec![connection.local_peer_id()];

    while let Some((peer_id, pk_bytes)) = connection.receive_public_key().await {
        println!("Recibida clave pública del jugador {}", peer_id);
        println!("PK antes:");
        let pk: PublicKey = P2PConnection::deserialize_canonical(&pk_bytes)?;
        println!("PK despues: {:?}", pk.to_string());
        player_keys.push(pk);
        all_peer_ids.push(peer_id.clone());

        // Si ya tenemos todas las claves públicas que esperamos, continuamos
        if player_keys.len() == 1 {
            break;
        }
    }

    // Selección del dealer mediante números aleatorios
    // Cada jugador envía un número del 1 al 10
    let my_number = rng.gen_range(1..=10);
    println!("Mi número para selección de dealer: {}", my_number);

    // Más adelante, cuando queramos enviar un token de revelación:
    // let reveal_token = andrija.compute_reveal_token(rng, &parameters, &deck[0])?;
    // let token_bytes = to_bytes![reveal_token]?;
    // connection.send_reveal_token(&token_bytes).await?;

    // // Y para recibir tokens de revelación:
    // let mut reveal_tokens = Vec::new();
    // while let Some((peer_id, token_bytes)) = connection.receive_reveal_token().await {
    //     println!("Recibido token de revelación del jugador {}", peer_id);
    //     let token = P2PConnection::deserialize(&token_bytes)?;
    //     reveal_tokens.push(token);

    //     if reveal_tokens.len() == 1 { // Esperamos 3 tokens más
    //         break;
    //     }
    // }

    // // Implementar lógica de apuestas usando GameAction
    // connection.send_action(GameAction::Bet(100)).await?;

    // // Esperar acciones de otros jugadores
    // while let Some((peer_id, action)) = connection.receive_action().await {
    //     match action {
    //         GameAction::Bet(amount) => println!("El jugador {} apostó {}", peer_id, amount),
    //         GameAction::Fold => println!("El jugador {} se retiró", peer_id),
    //     }
    // }
