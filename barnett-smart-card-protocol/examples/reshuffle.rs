...
struct Player {
    ...
    cards_public: Vec<Option<MaskedCard>>,
...
//     pub reveal_token: RevealToken,
//     pub reveal_proof: RevealProof,
//     pub public_key: PublicKey,
// }

// impl RevealTokenData {
//     pub fn new(reveal_token: RevealToken, reveal_proof: RevealProof, public_key: PublicKey) -> Self {
//         Self {
//             reveal_token,
//             reveal_proof,
//             public_key,
...
async fn main() -> anyhow::Result<()> {
    ...
    println!("Andrija cards: {:?}, {:?}", andrija.cards[0], andrija.cards[1]);
    // println!("Kobi cards: {:?}, {:?}", kobi_card1, kobi_card2);
    // println!("Nico cards: {:?}, {:?}", nico_card1, nico_card2);
    // println!("Tom cards: {:?}, {:?}", tom_card1, tom_card2);

    // Reshuffle. Tom leaves the game

    let players = vec![andrija.clone(), kobi.clone(), nico.clone()];
    ...
    // card_mapping debería quedar igual creo
    ...
    let (public, proof) = CardProtocol::remask_for_reshuffle(
        &mut prover,
        &mut r_prime,
        &parameters,
        &joint_pk,
        &new_deck,
        &andrija.cards_public,
        &andrija.sk,
        &andrija.pk,
        &m_list,
    )?;
    ...
    // Verificar el reshuffle revelando todas las cartas del mazo original
    println!("\n=== VERIFICACIÓN DEL RESHUFFLE ===");
    println!("Revelando cartas del mazo original:");
    
    // println!("mapping: {:?}", card_mapping);

    for (i, card) in new_deck.iter().take(52).enumerate() {
        let reveal_tokens: Vec<(RevealToken, RevealProof, PublicKey)> = players
            .iter()
            .map(|p| p.compute_reveal_token(rng, &parameters, card).unwrap())
            .collect();

        let original_card = match CardProtocol::unmask(&parameters, &reveal_tokens, card) {
            Ok(c) => {
                println!("Carta desenmascarada: {:?}", c);
                match card_mapping.get(&c) {
                    Some(classic_card) => format!("{:?}", classic_card),
                    None => "Carta inválida".to_string(),
                }
            }
            Err(e) => format!("Error al revelar: {:?}", e),
        };

        println!("Carta original {}: {}", i, original_card);
    }

    // Revelando cartas del mazo rebarajado
    println!("\nRevelando cartas del mazo rebarajado:");
    for (i, card) in reshuffled_deck.iter().take(52).enumerate() {
        let reveal_tokens: Vec<(RevealToken, RevealProof, PublicKey)> = players
            .iter()
            .map(|p| p.compute_reveal_token(rng, &parameters, card).unwrap())
            .collect();

        let reshuffled_card = match CardProtocol::unmask(&parameters, &reveal_tokens, card) {
            Ok(c) => {
                println!("Carta desenmascarada: {:?}", c);
                match card_mapping.get(&c) {
                    Some(classic_card) => format!("{:?}", classic_card),
        ...
        println!("Carta rebarajada {}: {}", i, reshuffled_card);
    }
    println!("==============================\n");
    ...
    // CARDS ARE SHUFFLED. ROUND OF THE GAME CAN BEGIN
    let final_deck = final_shuffled_deck;
